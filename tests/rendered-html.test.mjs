...
test("extension reveal measures its final layout and scrolls in the same animation", async () => {
  ...
  assert.match(source, /function ExtensionDescription/);
  assert.match(source, /className="extension-wide-header"/);
  assert.match(source, /className="extension-overview-label">OVERVIEW/);
  assert.match(css, /\.extension-wide-inner \{[^}]*flex-direction: column;/);
  assert.match(css, /\.extension-description \{[^}]*max-width: 820px;/);
  assert.match(css, /\.extension-description-section \{[^}]*border-top:/);
  assert.match(source, /description\.length > 2_200/);
  assert.match(source, /className="extension-description-columns"/);
  assert.match(css, /\.extension-description\.is-long \.extension-description-columns \{[^}]*grid-template-columns: repeat\(2,/);
});
...
test("server-renders Traditional Chinese as independent, linkable pages", async () => {
  ...
  assert.match(interestsHtml, /興趣與習慣/);
  assert.match(interestsHtml, /科學、宇宙與心理/);
  assert.match(interestsHtml, /投資、經濟與商業/);
  assert.match(interestsHtml, /水族造景與收藏/);
  assert.equal((interestsHtml.match(/class="interest-card glass"/g) ?? []).length, 8);
  assert.match(thinkingHtml, /我如何思考/);
  assert.match(thinkingHtml, /從第一性原理開始/);
  assert.match(thinkingHtml, /透過解釋深化理解/);
...
test("server-renders each default English navigation destination as a separate page", async () => {
  ...
  assert.match(interestsHtml, /Investing, economics &amp; business/);
